...
import com.abdel.mijnproject.network.MijnProAbdelInterface;
import com.abdel.mijnproject.network.MijnProDatabaseRetrofit;
import com.abdel.mijnproject.utils.GsonParser;
...
public class OnlineUserRepository  implements UserRepositoryInterface {
    ...
    public void login(String cnic, String password, ValidateCallBack validateCallBack) {
        MijnProAbdelInterface mijnProAbdelInterface= MijnProDatabaseRetrofit.getMijnProApiInterface();
        mijnProAbdelInterface.LoginRequest(Session.access_key,"1",cnic,password)
    ...
    public void signUp(User user, SaveCallBack saveCallBack) {
        ...
        String json=GsonParser.getInstance().GenerateJsonFromMode(user);
        MijnProAbdelInterface mijnProAbdelInterface=MijnProDatabaseRetrofit.getMijnProApiInterface();
        mijnProAbdelInterface.InsertnewUsers(user).enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if(response.isSuccessful()){
                    JSONObject jsonObject = null;
